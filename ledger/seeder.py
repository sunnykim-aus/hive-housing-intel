"""
Seeds the Policy Outcome Ledger with verified data on major Australian
housing programs. Data sourced from AHURI, Treasury, and government reports.
Each entry has: funding committed, targets set, actual outcomes, and the gap.
"""
...
PROGRAMS = [
    {
        ...
        "status": "Completed",
        "description": (
            "Australia's largest ever single investment in social housing. "
            "Part of the Rudd Government's economic stimulus response to the GFC. "
            "Funded new construction and repair of public housing dwellings across all states."
        ),
    ...
    },
    {
        ...
        "description": (
            "Provided annual incentives ($8,000–$11,000 per dwelling) to investors/developers "
            "to offer dwellings at 20% below market rent to eligible tenants. "
            "New allocations ceased in 2014 after change of government. "
            "Existing dwellings running to 2026."
        ),
    ...
    },
    {
        ...
        "funding_drawn_bn": None,
        "administering_agency": "Housing Australia (formerly NHFIC)",
        "program_type": "Bond Aggregation / CHP Finance",
        "geographic_scope": "Federal",
        "status": "Ongoing",
        "description": (
            "Bond aggregator providing cheaper long-term finance to registered community housing providers. "
            "Aggregates demand across CHPs to access wholesale bond markets at lower rates than individual CHPs can achieve. "
            "Transformed the CHP sector's access to capital. Renamed Housing Australia in 2023."
        ),
    ...
    },
    {
        ...
        "status": "Active",
        "description": (
            "A $10B off-budget fund whose returns (not principal) fund social and affordable housing. "
            "Managed by Future Fund. Distributes approximately $500M per year to Housing Australia "
            "for allocation to CHPs and states. First round allocations announced 2024. "
    ...
    },
    {
        ...
        "status": "Active",
        "description": (
            "Agreement between federal government, states, territories, and local government "
            "to deliver 1.2 million new well-located homes over 5 years from July 2024. "
            "Includes planning reform incentives, $3B Housing Support Program for states, "
            "and density bonuses. Currently tracking below target."
        ),
    ...
    },
    {
        ...
        "status": "Completed",
        "description": (
            "COVID-19 economic response. Provided $25,000 grants (later $15,000) "
            "to owner-occupiers building new homes or substantially renovating. "
            "Aimed at supporting the construction industry. Created significant demand surge "
            "but critics argue it inflated house prices and benefited middle-income households."
        ),
    ...
    },
    {
        ...
        "status": "Completed",
        "description": (
            "10-year investment in new housing and upgrades in remote Indigenous communities. "
            "Followed the NT Intervention. Transferred housing assets and management to NT Government. "
            "Significant investment but overcrowding in remote communities remains severe."
        ),
    ...
    },
    {
        ...
        "description": (
            "Government guarantees up to 15% of home loan deposit, allowing eligible first home buyers "
            "to purchase with 5% deposit. Expanded to Home Guarantee Scheme with multiple streams "
            "including Regional, Family, and Indigenous Home Guarantees. 50,000 places per year from 2022."
        ),
...
def seed(force=False):
    """Seed the ledger with program data. Skip if already seeded (unless force=True)."""
    init_db()
    conn = get_conn()
    existing = conn.execute("SELECT COUNT(*) FROM programs").fetchone()[0]
    if existing > 0 and not force:
        print(f"Ledger already has {existing} programs. Use force=True to re-seed.")
        conn.close()
        return
    ...
    if force:
        ...
        conn.execute("DELETE FROM extracted_insights")
        conn.execute("DELETE FROM programs")
        conn.commit()

    for p in PROGRAMS:
        targets = p.pop("targets", [])
        outcomes = p.pop("outcomes", [])

        cur = conn.execute("""
            INSERT INTO programs (name, short_name, announced_year, implemented_year, end_year,
                funding_committed_bn, funding_drawn_bn, administering_agency, program_type,
                geographic_scope, status, description, source_url)
            VALUES (:name, :short_name, :announced_year, :implemented_year, :end_year,
                :funding_committed_bn, :funding_drawn_bn, :administering_agency, :program_type,
                :geographic_scope, :status, :description, :source_url)
        """, p)
        ...
        for t in targets:
            conn.execute("""
                INSERT INTO targets (program_id, metric, target_value, target_unit, target_year, source)
                VALUES (?, ?, ?, ?, ?, ?)
            """, (program_id, t["metric"], t["target_value"], t["target_unit"],
                  t.get("target_year"), t.get("source")))
        ...
        for o in outcomes:
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (program_id, o["metric"], o["actual_value"], o["actual_unit"],
                  o.get("measurement_year"), o.get("confidence", "Medium"),
                  o.get("source_report"), o.get("notes")))

    conn.commit()
    conn.close()
    print(f"Ledger seeded with {len(PROGRAMS)} programs.")
